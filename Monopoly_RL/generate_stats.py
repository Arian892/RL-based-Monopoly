...
--------------
  ...
  python generate_stats.py --model ppo_hybrid_model.pt --games 500

  # 2-player game: focus PPO vs one fixed opponent
  python generate_stats.py --model my.pt --players 2 --p1 fixed-a

  # 3-player game: focus PPO vs pretrained rival + fixed-b
  python generate_stats.py --model my.pt --players 3 --p1 ppo:rival.pt --p2 fixed-b

  # 4-player game: all fixed opponents
  python generate_stats.py --model my.pt --p1 fixed-a --p2 fixed-b --p3 fixed-c

  # DDQN focus agent
  python generate_stats.py --algo ddqn --model ddqn.pt --games 200

  # Save stats to custom path, also generate plots
  python generate_stats.py --model my.pt --games 300 --out results/run1 --plot

  # Skip plots (stats JSON only)
  python generate_stats.py --model my.pt --games 500
"""
...
#
#   python generate_stats.py --replot stats_mymodel_4p_20240101_120000_stats.json
